...
public class ManageStudent extends Show {
	...
	private void freezeUnfreezeStudent() {
		if (selectedEntry != null) {
			if(isFrozen)
				teacher.sendChatMessage("/p user " + selectedEntry.getTitle().split("-")[0] + " group add _FROZEN_");	
			else 		
				teacher.sendChatMessage("/p user " + selectedEntry.getTitle().split("-")[0] + " group remove _FROZEN_");
	...
	private void switchMode() {
		if (selectedEntry != null) {
			teacher.sendChatMessage("/gamemode " + (isStudentInCreative ? "0 " : "1 ") + selectedEntry.getTitle().split("-")[0]);
